...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($calificaciones as $id_aspirante => $calificacion) {
        ...
        if (!empty($calificacion) && is_numeric($calificacion)) {
            ...
            // Actualizar la calificación y el promedio en la base de datos
            $query_update_calificacion = "UPDATE calificaciones 
                                          SET segundo_parcial = '$calificacion', 
                                              promedio = (primer_parcial + '$calificacion') / 3 
...
$query_calificaciones_asignadas = "SELECT id_aspirante, primer_parcial, segundo_parcial FROM calificaciones WHERE id_materia = $id_materia";
...
if(mysqli_num_rows($resultado_alumnos_con_calificacion) > 0){
        ...
        <div class="card card-body">
            <form method="POST" action="">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        while ($row_alumno_calificado = mysqli_fetch_assoc($resultado_alumnos_con_calificacion)) {
                            ...
                            echo "<td>";
                            // Verificar si ya hay una calificación del segundo parcial
                            if (isset($alumnos_con_calificacion[$row_alumno_calificado['id_aspirante']]['segundo_parcial'])) {
                                echo "<input type='number' name='calificaciones[" . $row_alumno_calificado['id_aspirante'] . "]' class='form-control' value='" . $alumnos_con_calificacion[$row_alumno_calificado['id_aspirante']]['segundo_parcial'] . "' required>";
                            } else {
                ...
                <div class="container p-4 d-flex justify-content-center">
                    <button type="submit" class="btn btn-primary form-control">Guardar Calificaciones del Segundo Parcial</button>
                </div>
            </form>
        </div>
    </div>
    ...
    ?>
    <div class="container p-4">
        <h1 class="mb-4">Ingresar Calificaciones del Segundo Parcial - Materia: <?php echo $nombre_materia; ?></h1>
        ...
        <div class="card card-body">
            <form method="POST" action="">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php
                        // Mostrar los alumnos en el formulario para ingresar nuevas calificaciones del segundo parcial
